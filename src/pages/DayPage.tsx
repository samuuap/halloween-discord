...
export default function DayPage() {
  ...
  return (
    <div
      className={clsx(
        "max-w-4xl mx-auto my-4 bg-black/95 backdrop-blur-md rounded-[36px] px-2 py-4 md:p-10 shadow-2xl relative overflow-hidden",
        "scale-[1.05] md:scale-100" // 🔍 zoom extra en móvil (tu petición)
      )}
    >
      {/* Cabecera */}
      ...
      {unlocked && (
        <>
          ...
          <div className="text-[5rem] md:text-7xl drop-shadow-lg mb-5">
            {cfg?.emojis ?? "❓"}
          </div>
          <div className="mt-2 text-4xl md:text-2xl text-purple-100 font-bold">
            1ª pista · Emojis
          </div>
              </div>
            </Card>
          ...
          className="bg-orange-600 hover:bg-orange-700 text-white font-black px-14 py-6 md:px-5 md:py-2 rounded-3xl shadow-2xl text-4xl md:text-2xl transition-all"
              >
          Siguiente pista →
              </button>
            ) : (
          ...
          className="bg-red-700 hover:bg-red-800 text-white font-black px-14 py-6 md:px-5 md:py-2 rounded-3xl shadow-2xl text-4xl md:text-2xl transition-all"
              >
          Rendirse 😵
              </button>
            )}
            <Link
              to="/"
              className="bg-gray-800 hover:bg-gray-900 text-white font-semibold px-14 py-6 md:px-5 md:py-2 rounded-3xl shadow-2xl text-4xl md:text-2xl transition-all"
            >
              ← Volver
            </Link>
          </div>
          ...
          <div className="flex flex-col items-center gap-10 mt-16 md:mt-6">
            ...
            <div className="text-3xl md:text-xl text-gray-200 text-center mb-6 whitespace-pre-line">
              {cfg?.synopsis?.trim()
                ? cfg.synopsis
            ...
            <button
              onClick={() => setGiveUpOpen(false)}
              className="mt-2 bg-purple-700 hover:bg-purple-800 px-14 py-5 md:px-5 md:py-2 text-white rounded-full shadow-lg text-3xl md:text-xl transition-all font-bold"
            >
              Cerrar
            </button>
          </div>
